...
//            saved.setId(String.valueOf(dto.getId()));
    ...
    private Map<String, Object> getKakaoAccessToken(String code) {
        ...
        params.add("code", code); // 프론트로부터 인가 코드 요청시 전달받은 코드값
    ...
    public String getKakaoProfileImg(String userId) {
        User foundUser = userRepository.findById(userId).orElseThrow();
        String accessToken = foundUser.getAccessToken();
        if(accessToken != null){
            return String.valueOf(getKakaoUserInfo(accessToken).getKakaoAccount().getProfile());
        }
        return null;
